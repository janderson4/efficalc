...
def plot(col, intervals, load_spaces):
    ...
    # and "load_spaces" is the number of vertical spaces in the PMM diagram
    ...
    for vert_count in range(1,load_spaces+1):
        ...
        for pos in (0,quarter*2,quarter*4):
            ...
            for j in range(3): coords[j][pos]=out[j]*mult[j]
            
        # get the output for theta=-90 and apply it in 2 points
        out=bisect_load.bisect(col,[math.pi/2,load_target],[-math.pi/2,c_guess])
        ...
        for i in range(1,quarter):
            out=bisection_factored.bisect(col,[lambda_target,
                               load_target], [theta_guesses[i-1],c_guess])

            theta_guesses[i-1]=out[3]

            c_guess=out[4]
            indices=[i,quarter*2-i,quarter*2+i,intervals-i] # the indices in 
            # the vector "coords" corresponding to the current point
            for pos,index in enumerate(indices):
                mult=[1 if (pos==0 or pos==3) else -1, 1 if pos<2 else -1, 1]
                for j in range(3): coords[j][index]=out[j]*mult[j]
