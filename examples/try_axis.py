...
STEEL_EPS2=0.005 # steel strain at PHI_FLEXURE
...
def try_axis(col,theta,c):
    ...
    a=col.beta1*c
    epsilon=0.000001 # acceptable error for considering the neutral axis to
    ...
    if theta>-math.pi/2+epsilon:
        ...
        intersects[0]=-col.half_h<left_y and left_y<col.half_h
        
        right_y=col.half_h-a/math.cos(theta)
        intersects[2]=-col.half_h<right_y and right_y<col.half_h
    
    if theta<-epsilon:
        top_x=col.half_w+a/math.sin(theta)
        intersects[1]=-col.half_w<top_x and top_x<col.half_w
        ...
        intersects[3]=-col.half_w<bot_x and bot_x<col.half_w
    ...
    if not any(intersects): # the whole concrete section is in compression
        ...
        def add_axial_moment(pt_a,pt_b,pt_c):
            ...
            for i in range(2): mn[i]+=tri_area*red_fc*centr[1-i]
    ...
    def add_bar(coords):
        ...
        # the neutral axis
        offset=normal[0]*(col.half_w-coords[0])+normal[1]*(col.half_h-coords[1])
        ...
        pn-=force
        for i in range(2): mn[i]-=force*coords[1-i]
    ...
    phi=min(PHI_FLEXURE,max(col.PHI_COMP,col.PHI_COMP+(PHI_FLEXURE-
                        col.PHI_COMP)*(steel_max_strain-col.steel_yield)/
                        (STEEL_EPS2-col.steel_yield)))
    phi_pn=phi*pn
