...
def rebar_area(size):
    for pos,name in enumerate(REBAR_SIZES):
        if size==name: return REBAR_AREAS[pos]
    raise Exception("invalid rebar size")
    
def rebar_diameter(size):
    for pos,name in enumerate(REBAR_SIZES):
        if size==name: return REBAR_DIAMETERS[pos]
    raise Exception("invalid rebar size")

class Column:
    def __init__(self,w,h,bar_size,bar_cover,bars_x,bars_y,
                 fc,fy,cover_to_center,spiral_reinf):
        self.w=w
        self.h=h
        self.half_w=w/2
        self.half_h=h/2
        ...
        self.fc=fc
        self.beta1=max(0.65,min(0.85,0.85-.05/1000*(self.fc-4000))) # the ratio 
        ...
        self.max_pn=max_tens_comp.max_pn(self)
        self.max_phi_pn=max_tens_comp.max_phi_pn(self)
        
        # max tension capacity
        self.min_pn=max_tens_comp.min_pn(self)
        self.min_phi_pn=max_tens_comp.min_phi_pn(self)
